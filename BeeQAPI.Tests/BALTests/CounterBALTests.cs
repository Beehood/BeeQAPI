...
{
    public class CounterBALTests
    {
        private readonly Mock<IDAL_Counter> _mockDal;
        private readonly BAL_Counter _bal;

        public CounterBALTests()
        {
            _mockDal = new Mock<IDAL_Counter>();
            _bal = new BAL_Counter(_mockDal.Object);
        }
        ...
        public async Task GetAll_Should_Return_Data_When_Role_Is_Valid()
        {
            ...
            _mockDal.Setup(x => x.GetAll(
                ...
                .ReturnsAsync(new APIGetResponseModel<List<CounterModel>>
                {
                    IsSuccess = true,
        ...
        public async Task GetById_Should_Return_Data_When_Role_Is_Valid()
        {
            ...
            _mockDal.Setup(x => x.GetById(
                    ...
                    It.IsAny<string>(),
                    It.IsAny<IDbTransaction>()))
                .ReturnsAsync(new APIGetResponseModel<CounterModel>
                {
                    IsSuccess = true,
        ...
        {
            // Arrange
            CounterRequestDto request = null;
        ...
        public async Task Create_Should_Return_ValidationErrors_When_Required_Fields_Are_Missing()
        {
            ...
            // Assert
            Assert.False(result.IsSuccess);
            Assert.Contains("Counter Name is required", result.ErrorMsgs);
        ...
        public async Task Update_Should_Return_Error_When_Request_Is_Invalid()
        {
            ...
            // Assert
            Assert.False(result.IsSuccess);
            Assert.Contains("Invalid counter data.", result.ErrorMsgs);
        }

        [Fact]
        public async Task Update_Should_Return_Error_When_CounterName_Is_Missing()
        {
            // Arrange
            var request = new CounterRequestDto
            {
        ...
        public async Task ChangeStatus_Should_Return_Error_When_Id_Is_Invalid()
        {
            ...
            // Assert
            Assert.False(result.IsSuccess);
            Assert.Contains("Invalid counter ID.", result.ErrorMsgs);
        }
