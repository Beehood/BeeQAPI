...

    {
...

        {
...

            {
                ...
                _logger.LogInformation("NextTokenPreview requested by {Email}", email);
...

                {
                    ...
                    _logger.LogWarning("Unauthorized NextTokenPreview attempt by {Email}", email);
                ...
                response = await _dal.NextTokenPreview(request, roles, email, transaction);

                if (response.IsSuccess)

                {

                    _logger.LogInformation("NextTokenPreview completed successfully for {Email}", email);
...

            {
                ...
                response.IsSuccess = false; _logger.LogError(ex, "BAL NextTokenPreview Error");

                response.ErrorMsgs.Add("Error while previewing next token");
